    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['warehouse:wmsOrderTaskDesc:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['warehouse:wmsOrderTaskDesc:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['warehouse:wmsOrderTaskDesc:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['warehouse:wmsOrderTaskDesc:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="wmsOrderTaskDescList" @selection-change="handleSelectionChange">
...
<script>
import { listWmsOrderTaskDesc, getWmsOrderTaskDesc, delWmsOrderTaskDesc, addWmsOrderTaskDesc, updateWmsOrderTaskDesc } from "@/api/warehouse/wmsOrderTaskDesc";

export default {
  name: "WmsOrderTaskDesc",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWmsOrderTaskDesc(this.queryParams).then(response => {
        this.wmsOrderTaskDescList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getWmsOrderTaskDesc(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateWmsOrderTaskDesc(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addWmsOrderTaskDesc(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除库存工作单编号为"' + ids + '"的数据项？').then(function() {
        return delWmsOrderTaskDesc(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('warehouse/wmsOrderTaskDesc/export', {
        ...this.queryParams
      }, `wmsOrderTaskDesc_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
