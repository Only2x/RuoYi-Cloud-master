    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['warehouse:wmsPurchaseDesc:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['warehouse:wmsPurchaseDesc:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['warehouse:wmsPurchaseDesc:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['warehouse:wmsPurchaseDesc:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="wmsPurchaseDescList" @selection-change="handleSelectionChange">
...
<script>
import { listWmsPurchaseDesc, getWmsPurchaseDesc, delWmsPurchaseDesc, addWmsPurchaseDesc, updateWmsPurchaseDesc } from "@/api/warehouse/wmsPurchaseDesc";

export default {
  name: "WmsPurchaseDesc",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWmsPurchaseDesc(this.queryParams).then(response => {
        this.wmsPurchaseDescList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getWmsPurchaseDesc(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateWmsPurchaseDesc(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addWmsPurchaseDesc(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('warehouse/wmsPurchaseDesc/export', {
        ...this.queryParams
      }, `wmsPurchaseDesc_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
