    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['warehouse:wmsSku:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['warehouse:wmsSku:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['warehouse:wmsSku:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['warehouse:wmsSku:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="wmsSkuList" @selection-change="handleSelectionChange">
...
<script>
import { listWmsSku, getWmsSku, delWmsSku, addWmsSku, updateWmsSku } from "@/api/warehouse/wmsSku";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWmsSku(this.queryParams).then(response => {
        this.wmsSkuList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateWmsSku(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addWmsSku(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('warehouse/wmsSku/export', {
        ...this.queryParams
      }, `wmsSku_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
