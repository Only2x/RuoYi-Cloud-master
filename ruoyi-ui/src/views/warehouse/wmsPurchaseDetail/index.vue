    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['warehouse:wmsPurchaseDetail:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['warehouse:wmsPurchaseDetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['warehouse:wmsPurchaseDetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['warehouse:wmsPurchaseDetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="wmsPurchaseDetailList" @selection-change="handleSelectionChange">
...
<script>
import { listWmsPurchaseDetail, getWmsPurchaseDetail, delWmsPurchaseDetail, addWmsPurchaseDetail, updateWmsPurchaseDetail } from "@/api/warehouse/wmsPurchaseDetail";

export default {
  name: "WmsPurchaseDetail",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWmsPurchaseDetail(this.queryParams).then(response => {
        this.wmsPurchaseDetailList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getWmsPurchaseDetail(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateWmsPurchaseDetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addWmsPurchaseDetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除采购单详情编号为"' + ids + '"的数据项？').then(function() {
        return delWmsPurchaseDetail(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('warehouse/wmsPurchaseDetail/export', {
        ...this.queryParams
      }, `wmsPurchaseDetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
