    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['product:spuvalue:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['product:spuvalue:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['product:spuvalue:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['product:spuvalue:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="spuvalueList" @selection-change="handleSelectionChange">
...
<script>
import { listSpuvalue, getSpuvalue, delSpuvalue, addSpuvalue, updateSpuvalue } from "@/api/product/spuvalue";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSpuvalue(this.queryParams).then(response => {
        this.spuvalueList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSpuvalue(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSpuvalue(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('product/spuvalue/export', {
        ...this.queryParams
      }, `spuvalue_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
