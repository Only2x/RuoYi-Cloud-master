    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['product:attrgrouprelation:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['product:attrgrouprelation:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['product:attrgrouprelation:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['product:attrgrouprelation:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listAttrgrouprelation, getAttrgrouprelation, delAttrgrouprelation, addAttrgrouprelation, updateAttrgrouprelation } from "@/api/product/attrgrouprelation";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAttrgrouprelation(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getAttrgrouprelation(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateAttrgrouprelation(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAttrgrouprelation(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('product/attrgrouprelation/export', {
        ...this.queryParams
      }, `attrgrouprelation_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
