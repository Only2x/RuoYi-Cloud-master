    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['product:skuvalue:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['product:skuvalue:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['product:skuvalue:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['product:skuvalue:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="skuvalueList" @selection-change="handleSelectionChange">
...
<script>
import { listSkuvalue, getSkuvalue, delSkuvalue, addSkuvalue, updateSkuvalue } from "@/api/product/skuvalue";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSkuvalue(this.queryParams).then(response => {
        this.skuvalueList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSkuvalue(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSkuvalue(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('product/skuvalue/export', {
        ...this.queryParams
      }, `skuvalue_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
