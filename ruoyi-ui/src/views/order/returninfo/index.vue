    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['order:returninfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['order:returninfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['order:returninfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['order:returninfo:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listReturninfo, getReturninfo, delReturninfo, addReturninfo, updateReturninfo } from "@/api/order/returninfo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listReturninfo(this.queryParams).then(response => {
        this.returninfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getReturninfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateReturninfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addReturninfo(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('order/returninfo/export', {
        ...this.queryParams
      }, `returninfo_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
