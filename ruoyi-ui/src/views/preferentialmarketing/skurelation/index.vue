    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['preferentialmarketing:skurelation:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['preferentialmarketing:skurelation:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['preferentialmarketing:skurelation:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['preferentialmarketing:skurelation:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="skurelationList" @selection-change="handleSelectionChange">
...
<script>
import { listSkurelation, getSkurelation, delSkurelation, addSkurelation, updateSkurelation } from "@/api/preferentialmarketing/skurelation";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSkurelation(this.queryParams).then(response => {
        this.skurelationList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSkurelation(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSkurelation(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSkurelation(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除秒杀活动商品关联编号为"' + ids + '"的数据项？').then(function() {
        return delSkurelation(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('preferentialmarketing/skurelation/export', {
        ...this.queryParams
      }, `skurelation_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
