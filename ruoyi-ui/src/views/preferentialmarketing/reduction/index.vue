    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['preferentialmarketing:reduction:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['preferentialmarketing:reduction:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['preferentialmarketing:reduction:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['preferentialmarketing:reduction:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listReduction, getReduction, delReduction, addReduction, updateReduction } from "@/api/preferentialmarketing/reduction";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listReduction(this.queryParams).then(response => {
        this.reductionList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateReduction(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addReduction(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除商品满减信息编号为"' + ids + '"的数据项？').then(function() {
        return delReduction(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('preferentialmarketing/reduction/export', {
        ...this.queryParams
