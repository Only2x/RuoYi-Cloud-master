    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['preferentialmarketing:bounds:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['preferentialmarketing:bounds:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['preferentialmarketing:bounds:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['preferentialmarketing:bounds:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listBounds, getBounds, delBounds, addBounds, updateBounds } from "@/api/preferentialmarketing/bounds";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBounds(this.queryParams).then(response => {
        this.boundsList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateBounds(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addBounds(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('preferentialmarketing/bounds/export', {
        ...this.queryParams
